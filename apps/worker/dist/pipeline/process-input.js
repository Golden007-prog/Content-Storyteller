...
const PLATFORM_GUIDANCE = {
    [shared_1.Platform.InstagramReel]: '15-second vertical reel format. Hook-first pacing — grab attention in the first 2 seconds. ' +
    ...
    [shared_1.Platform.LinkedInLaunchPost]: 'Professional long-form format. Thought-leadership angle with data-driven insights. ' +
    ...
    [shared_1.Platform.XTwitterThread]: 'Thread format with numbered posts (1/N). Punchy hooks on each post to maintain scroll-through. ' +
        'Concise, high-impact sentences. Use line breaks for readability. ' +
        'First tweet must stand alone as a compelling hook.',
    [shared_1.Platform.GeneralPromoPackage]: 'Versatile multi-format package suitable for cross-platform use. ' +
...
const TONE_DIRECTION = {
    [shared_1.Tone.Cinematic]: 'Cinematic tone — sweeping, dramatic language. Evoke emotion through vivid imagery and storytelling. ' +
        'Think movie trailer energy: bold statements, atmospheric visuals, epic pacing.',
    [shared_1.Tone.Punchy]: 'Punchy tone — short, impactful sentences. High energy, direct, and memorable. ' +
        'Use power words, strong verbs, and rhythmic cadence. No fluff.',
    [shared_1.Tone.Sleek]: 'Sleek tone — minimal, refined, and modern. Clean aesthetic with understated confidence. ' +
        'Less is more. Elegant word choices, whitespace-friendly layouts, premium feel.',
    [shared_1.Tone.Professional]: 'Professional tone — formal, authoritative, and trustworthy. ' +
 ...
 */
function buildCreativeDirectorPrompt(promptText, platform, tone) {
    const platformGuide = PLATFORM_GUIDANCE[platform] || PLATFORM_GUIDANCE[shared_1.Platform.GeneralPromoPackage];
    const toneGuide = TONE_DIRECTION[tone] || TONE_DIRECTION[shared_1.Tone.Professional];
...
class ProcessInput {
    name = 'ProcessInput';
    jobState = shared_1.JobState.ProcessingInput;
    async execute(context) {
        ...
        try {
            ...
            const platform = job?.platform || shared_1.Platform.GeneralPromoPackage;
            const tone = job?.tone || shared_1.Tone.Professional;
            ...
            await (0, firestore_1.updateJobState)(context.jobId, this.jobState, { creativeBrief });
...
//# sourceMappingURL=process-input.js.map